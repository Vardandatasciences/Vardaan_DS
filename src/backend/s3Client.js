...
class RenderS3Client {
    ...
    // Save media record to media_library table
    async saveMediaRecord(fileInfo, category = 'general', fileType = null, displayConfig = {}) {
        if (!this.dbPool) {
            console.warn("⚠️ No database connection available for media record");
            return null;
        }
        ...
        try {
            ...
            if (!fileType) {
                const ext = path.extname(fileInfo.original_name || fileInfo.storedName).toLowerCase();
                fileType = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'bmp'].includes(ext) ? 'image' : 'video';
            }
            
            // Check if display fields exist, if not add them
            try {
                await connection.execute(`
            ...
            const query = `
                ...
                (original_name, s3_url, file_type, category, uploaded_by, uploaded_at, 
                 display_name, display_order, is_active, content_disposition)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            `;
            
            const params = [
                fileInfo.original_name || fileInfo.storedName,
                fileInfo.url,
                fileType,
                category,
                'system',
                new Date(),
                displayConfig.display_name || null,
                displayConfig.display_order || 0,
                displayConfig.is_active !== undefined ? displayConfig.is_active : 1,
                displayConfig.content_disposition || 'inline'
            ];
            
            const [result] = await connection.execute(query, params);
            console.log(`📝 Media record saved: ID ${result.insertId}`);
            return result.insertId;
            
        } catch (error) {
            console.error("❌ Media record save error:", error);
            return null;
        } finally {
            if (connection) connection.release();
        }
    ...
    // Upload with media library integration
    async uploadWithMediaLibrary(filePath, userId = "default-user", customFileName = null, category = 'general', displayConfig = {}) {
        const uploadResult = await this.upload(filePath, userId, customFileName);
        
        if (uploadResult.success && uploadResult.file_info) {
            // Save to media library with display configuration
            await this.saveMediaRecord(uploadResult.file_info, category, null, displayConfig);
        }
